...
- [Divinikey] If product-page enrichment fails and the computed unit price remains outside `$0.15-$1.50`, skip the row instead of saving a quantity-`1` active listing. Divinikey often states quantity as `Size 18 Set`, `(18 Pack)`, or `18 included in each pack` on the product page.
...
- [Core Engine] `fetch_html()` must return a string or raise `RuntimeError`; never return `None`. Scrapers pass its output directly into BeautifulSoup, so `None` causes `TypeError: Incoming markup is of an invalid type`.
